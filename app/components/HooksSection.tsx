...
const HOOK_TYPES: { value: keyof Hooks; label: string; description: string }[] = [
	...
	{ value: "PostToolUse", label: "Post Tool Use", description: "Run after a tool executes" },
	{ value: "Notification", label: "Notification", description: "Run when a notification is sent" },
	{ value: "UserPromptSubmit", label: "User Prompt Submit", description: "Run when user submits a prompt" },
];
			...
			<Flex justify="between" align="start" mb="3">
				<Box style={{ flex: 1 }}>
					...
					<TextField.Root
						...
						value={entry.matcher || ""}
						onChange={(e) => onChange({ ...entry, matcher: e.target.value || undefined })}
						style={{ fontFamily: "monospace" }}
					/>
				</Box>
				<IconButton size="1" variant="ghost" color="red" onClick={onRemove} ml="2">
					<Cross2Icon />
				</IconButton>
			</Flex>
			...
			<Flex direction="column" gap="2">
				{entry.hooks.map((hook, index) => (
					<Flex key={index} gap="2" align="center">
						...
						/>
						<IconButton size="2" variant="soft" color="red" onClick={() => removeHook(index)}>
							<Cross2Icon />
						</IconButton>
					</Flex>
				))}
				<Flex gap="2">
					<TextField.Root
						...
						style={{ flex: 1, fontFamily: "monospace" }}
					/>
					<IconButton size="2" variant="soft" onClick={addHook} disabled={!newHook.trim()}>
						<PlusIcon />
					</IconButton>
				</Flex>
			</Flex>
...
export function HooksSection({ settings, onChange }: HooksSectionProps) {
	...
	const updateHooks = (newHooks: Hooks) => {
		...
		onChange({ ...settings, hooks: Object.keys(cleaned).length > 0 ? cleaned : undefined });
	};
	...
	return (
		<Flex direction="column" gap="5">
			...
			<Box>
				...
				<Select.Root value={selectedType} onValueChange={(v) => setSelectedType(v as keyof Hooks)}>
						...
						{HOOK_TYPES.map((type) => (
							<Select.Item key={type.value} value={type.value}>
								...
								{(hooks[type.value]?.length || 0) > 0 && (
									<Text color="gray" ml="2">({hooks[type.value]?.length})</Text>
								)}
							</Select.Item>
						))}
					</Select.Content>
